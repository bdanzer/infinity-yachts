...
function create_boat($image, $name, $price_from, $price_to, $guests, $staterooms, $boat_type, $length, $url) {
	echo '<div class="danzerpress-col-2 wow fadeIn danzerpress-flex-column" style="margin-bottom: 20px;">';
		echo '<div class="danzerpress-box" style="line-height: inherit; padding: 0px;">';
			echo '<div style="overflow:hidden; line-height: 0px;"><a href="' . $url . '"><img style="height: 100%;max-height: 200px;object-fit: cover;width: 100%;" src="' . $image . '"></a></div>';
			echo '<table class="yacht-specs danzerpress-box danzerpress-white danzerpress-shadow-3" style="margin:0px;">';
				echo '<tr><td style="">Name</td><td style="color:black;font-weight:700;">' . ucwords($name) . '</td></tr>';
			...
			echo '</table>';
			echo '<a style="display:block;border-radius:0px;" class="danzerpress-button-modern" href="' . $url . '">View Yacht</a>';
		echo '</div>';
	echo '</div>';
}
...
function check_for_xml_updates() {
	...
	foreach ($xml_snapins as $value) {
		...
		if(in_array($value->yachtId, $option_value) && get_post_status($post_id) != 'trash' && (int)get_field('overide_cya', $post_id) != 1) {
			...
			//upload new image
			$return = media_sideload_image($cya_feed_content['image'], $yacht_id, $cya_feed_content['desc'], 'id');

			//set new picture to post
			set_post_thumbnail($yacht_id, $return);
